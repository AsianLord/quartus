...
always@(posedge clk) begin
	...
	if ((red_beacon_detect && prev_detect_high_r && prev_high_r && prev_high_r2 ) & in_valid) begin
		if (x < x_min_r) x_min_r <= x;
		if (x > x_max_r) x_max_r <= x;
      if (y < y_min_r) y_min_r <= y;
		y_max_r <= y;
	end
	if ((orange_beacon_detect && prev_detect_high_y && prev_high_y && prev_high_y2) & in_valid) begin
		if (x < x_min_y) x_min_y <= x;
		if (x > x_max_y) x_max_y <= x;
      if (y < y_min_y) y_min_y <= y;
		y_max_y <= y;
	end
	if ((blue_beacon_detect && prev_detect_high_b && prev_high_b && prev_high_b2) & in_valid) begin
